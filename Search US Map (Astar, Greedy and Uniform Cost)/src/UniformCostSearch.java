...
public final class UniformCostSearch {

	public static void uniformCostSearch(int sourceCityIndex, int destinationCityIndex, City[] cities, int[][] adjMat){
		...
		while(!cityQueue.isEmpty()){
			try{
				...
				if(expandedNode.getCityIndex() == destinationCityIndex){
					...
					System.out.println("Sequence of nodes expanded = " + pathExpansion);
				...
				for(int i = 0; i < adjMat[0].length; i++){
					if(adjNodes.containsKey(i)){
						...
						// Setting up the heuristic path cost as 0 since uniform cost search algorithm
						// does not take into account the heuristic path
						double heuristicCost = 0;
						double pathCost = adjNodes.get(i) + expandedNode.getPathCost();  
						
						// Calculate the cost of the node
						adjCity.setPathCost(pathCost);
						adjCity.setTotalPathCost(heuristicCost + pathCost);
